...
class AdddataClass extends React.Component{
    ...
    handleData = (e) => {
        ...
        if(name === 'languages'){
            ...
            this.setState({formData: {...this.state.formData, [name]: lng}});
        }
        else if(name === 'image'){
            if (files && files[0]){
                this.setState({formData: {...this.state.formData, [name]: URL.createObjectURL(files[0])}});
            }
        }
        else{
    ...
    submitData = () => {
        ...
        if(this.state.formData.name.match(/^[a-zA-Z]+(\s[a-zA-Z]+)?$/) && this.state.formData.email.match(/^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$/) && this.state.formData.contact.match(/^\d{10}$/) && this.state.formData.dob && this.state.formData.gender && lng.length && this.state.formData.image && this.state.formData.nationality !== 'Nationality'){
            this.props.submitAllData(this.state.formData);

            this.props.navigate('/');
            this.resetData();
        }
        else{
            !this.state.formData.name && (alertIcons[0].style.opacity = 1);
            !this.state.formData.email && (alertIcons[1].style.opacity = 1);
            !this.state.formData.contact && (alertIcons[2].style.opacity = 1);
            !this.state.formData.dob && (alertIcons[3].style.opacity = 1);
            !this.state.formData.gender && (alertIcons[4].style.opacity = 1);
            !lng.length && (alertIcons[5].style.opacity = 1);
            !this.state.formData.image && (alertIcons[6].style.opacity = 1);
            this.state.formData.nationality === 'Nationality' && (alertIcons[7].style.opacity = 1);
        }
    };
    ...
    resetData = () => {
        ...
        this.setState({formData: {...this.state.formData, name: '', email: '', contact: '',gender: '', dob: '', languages: [], nationality: 'Nationality', image: ''}});
                                ...
                                <div className={`${style.formwrapper}`}>
                                    ...
                                    <div className={`${style.fieldwrap} ${style.fieldwrap1} d-flex justify-content-between`}>
                                        <div className={`${style.field} ${style.field1} me-4`}>
                                            ...
                                            <input type={'text'} id={'name'} name={'name'} value={this.state.formData.name} placeholder={'Enter Name...'} autoComplete={'off'}
                                        ...
                                        <div className={`${style.field} ${style.field2} ms-4`}>
                                            ...
                                            <input type={'email'} id={'email'} name={'email'} value={this.state.formData.email} placeholder={'Enter Email...'} autoComplete={'off'}
                                                   onChange={(e) => this.handleData(e)}
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <div className={`${style.fieldwrap} ${style.fieldwrap2} d-flex justify-content-between`}>
                                        <div className={`${style.field} ${style.field3}`}>
                                            ...
                                            <input type={'number'} id={'contact'} name={'contact'} value={this.state.formData.contact} placeholder={'Enter Contact...'} autoComplete={'off'}
                                                   onChange={(e) => this.handleData(e)}
                                            />
                                        </div>
                                        ...
                                        <div className={`${style.field} ${style.field4}`}>
                                            ...
                                            <input type={'date'} id={'dob'} name={'dob'} value={this.state.formData.dob}
                                                   onChange={(e) => this.handleData(e)}
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <div className={`${style.fieldwrap} ${style.fieldwrap4} d-flex justify-content-between`}>
                                        ...
                                        <div className={`${style.field} ${style.field8}`}>
                                            ...
                                            <select id={'nationality'} value={this.state.formData.nationality} name={'nationality'}
                                                    onChange={(e) => this.handleData(e)}
                                            >
                                    ...
                                    <div className={`${style.field} ${style.field9} text-center mt-4`}>
                                        <button onClick={this.submitData} className={`${style.buttons} ${style.submit}`}>SUBMIT</button>
                                        <button onClick={this.resetData} className={`${style.buttons} ${style.reset}`} >RESET</button>
                                        <button className={`${style.buttons} ${style.reset}`} onClick={() => this.props.navigate('/')}>CANCEL</button>
                                    </div>
...
export default connect(null, UseDispatch)(AdddataClass);